...
- ⚠️ Table 4.2 (tiers summary) was OCR-interleaved column-wise but is recoverable; **Table 4.4 (below) is intact** in the raw text and is the source for per-LLM HCR/JIR.

## 11b. Direct-LLM baselines — Table 4.4 (lines 4593–4612, intact)
Per-model values used by the left-column **Hallucinated-Citation Rate bar chart**:

| System | Params | CitF1 | HCR | JIR |
|---|---|---|---|---|
| Gemini 2.5 Flash | — | 0.186 | **0.008** | 0.375 |
| Llama 3.2 | 3B | 0.129 | **0.492** | 0.400 |
| Gemma 2 | 9B | 0.120 | **0.725** | 0.175 |
| Llama 3.3 | 70B | 0.100 | **0.803** | 0.350 |
| Qwen 2 | 7B | 0.085 | **0.701** | 0.675 |
| Llama 3.1 | 8B | 0.068 | **0.775** | 0.500 |
| Aya Expanse | 8B | 0.030 | **0.811** | 0.325 |
| AKN-RLM (locked) | — | 0.305 | **0.000** | 0.000 |

Narrative anchors for the chart's caption claims:
- "the largest baseline, Llama 3.3 at 70B, is also the one with the highest hallucinated citation rate at HCR = 0.803" — lines 4676–4678 (also 6030–6032).
- Gemini paradox: "achieves its low HCR primarily by **declining to cite articles at all**" — lines 4686–4692.
- "Six of the seven direct LLM baselines have **JIR above 0.30**, meaning they import foreign-law concepts on **more than twelve of the forty** infection traps" — lines 4701–4703.
- "Up to 4 of 5 cited articles fabricated" = HCR 0.811 (Aya) / 0.803 (Llama 3.3) rounded — Table 4.4.

## 11c. Example questions printed in the left column (verbatim from the benchmark)
Source: `latest_dataset/AlgerianLegalBench_v3.0_final.json` (`questions[]` by `id`):
- **civ_ra_q01** (rule_application, ar, answerable) — "بيّن الأساس القانوني لمبدأ خضوع القاضي للنص التشريعي، وحدود سلطته عند غياب النص…" (truncated after second clause); expected articles: Civil Code (doc `75-8_1975-09-26`, law_name_ar "القانون المدني") **Art. 1, 2** → poster tag "Civil Code Art. 1–2".
- **fam_tf_q01** (temporal_factual, ar, answerable) — "كيف تغيرت شروط الخلع في قانون الأسرة بعد تعديل 2005؟" (poster adds shadda display form تغيّرت); expected: doc `84-11_1984-06-09` **Art. 54** → tag "Ord. 05-02 → Art. 54" (amendment instrument per thesis lines 2486–2544).
- **lab_un_q03** (unanswerable infection trap, ar) — "هل يحق لصاحب العمل في الجزائر فصل العامل بحرية تامة دون إبداء أسباب (at-will employment)؟" (poster shortens parenthetical to "(at-will)") → tag "foreign-doctrine trap → abstain".
