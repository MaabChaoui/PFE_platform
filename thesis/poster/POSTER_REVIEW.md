...
## v3 (2026-06-10) — left column rebuilt: sketch structure + "paper" aesthetic
Structure follows the author's sketch and the ENSIA directive (Problem Statement /
Objectives / Significance & Impact): **problem → real examples → LLM-failure figure →
hard case → objectives → significance**. After an author pass ("too vibecoded — make
it LaTeX-like, a flow rather than blocks"), the column was restyled as a **single white
paper sheet**: Liberation Serif throughout, numbered subsections `1.1–1.6` divided by
hairline rules, justified prose — no rounded outline-cards, no icon tiles.

- **1.1** Problem paragraph (expanded per author request) + the author-supplied ChatGPT
  spot illustration floated right. The PNG had a *baked-in* transparency checkerboard
  (RGB, ~242-gray squares); cleaned in PIL (light low-chroma pixels → alpha, recropped)
  to `assets/generated/problem_spot_clean.png`. Keywords line in small caps between
  hairlines, "or refused" in red.
- **1.2** Three *verbatim* benchmark questions (`civ_ra_q01`, `fam_tf_q01`, `lab_un_q03`
  — `EXTRACTED_FACTS.md` §11c) as RTL serif block quotes (Noto Naskh Arabic) with thin
  colour-coded right rules + italic demand annotations.
- **1.3** **Fig. 1** — per-LLM HCR from the intact Table 4.4 (§11b): flat red bars
  0.492→0.811 ordered by params (3B→70B ⇒ "scale doesn't fix law"), L-frame axes +
  0.25 gridlines, dashed divider, green AKN-RLM 0.000; journal-style justified caption
  (Gemini's 0.008 = declining-to-cite, explained there; its bar intentionally not drawn).
  *Bug fixed during build:* percentage bar heights collapsed until `.hbar` got
  an explicit `height:100%` (flex column with `auto` height ⇒ `%` resolves to 0).
- **1.5/1.6** Objectives as a borderless 2×2 check list; significance as a numbered
  contributions list with **AKN corpus + KG** and **AlgerianLegalBench** split into
  nested sub-entries (author request).
- QC re-run: 1 page, A3 (841.92×1191.12 pt), chart + Arabic re-verified at 300 dpi.
