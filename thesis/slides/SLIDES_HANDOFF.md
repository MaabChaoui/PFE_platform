...
## Current Status
Deck rebuilt to the supervisor's numbered outline (v5/v5.1, 2026-06-12). Current order (34): Title · Hook · Outline · **L&L divider** · Legal-meaning · The-problem · Algerian context · Hallucination · **Foundations divider (now with pillars outline: 4 classical columns + AKN-RLM entablature + Arabic-NLP stylobate)** · Arabic NLP · Pillars (RAG, KG, AM, RLM) · **Synthesis+Gaps merged** (gap-card layout, SOTA folded in) · **AKN-RLM divider** · **17 Context** (boundary diagram: RLM supervisor center, AI-Grid LLMs left, corpus→indices/KG right, user below, benchmark outside as evaluator, contributions i/ii/iii) · **18 Corpus+KG** (pipeline + eId XML pane + law-student/senior-expert multi-pass validation card) · **19 Benchmark** (query-type bars, difficulty/language stats, 3-step annotation protocol, κ=0.829) · **20 Query dispatcher** (flow strip + 8 behaviour cards) · 21 Architecture · 22 **Generative inside / deterministic outside** (nested shell) · 23 **Bounded recursion** (methodology RLM: root-controller loop, depth ≤ 3, gap-probe/coverage/workers/audit) · 24 **Toulmin-style ADUs** (ground→claim + warrant + rebuttal diagram, feeds gate iii, AMF 0.471, scope footnote) · 25 Gate · 26 **Deployment+objectives** · 27 **Evaluation setup & metrics** · **Results divider** · **Main results (1/2)** (bignums + tier-best bars ×1.64/×1.74/×2.9) · **Interpretation (2/2)** (compact staircase + 5 readings) · **Conclusion divider** · Conclusion ledger · **Limitations & future work** · Thanks.
